...
export const TransactionTable = ({ transactions = [] }) => {
  const TableHead = ({ children }) => (
    <section className="sticky  top-0 z-20 text-[10px]   bg-indigo-100   grid md:grid-cols-12 grid-cols-8 items-center   py-1 gap-1  md:text-base font-semibold text-indigo-900">
      {children}
    </section>
  );
  const TableRowHead = ({ lable, customClass }) => (
    <div className={`col-span-1 px-1  ${customClass}`}>{lable}</div>
  );
  return (
    <section className="flex-1 rounded-xl  md:text-md bg-white   shadow-md flex flex-col overflow-hidden">
      ...
      <TableHead>
        ...
        <TableRowHead lable={"Date"} customClass={"  md:col-span-2 "} />
        <TableRowHead
          lable={"Description"}
